 ...
 *	
 * --Modified from above; very basic monte-carlo approach: if a play/move is valid, 
 * randomly decide whether or not to do it. Namely, randomness occurs in the following ways:
 *  - randomly decide whether or not to take the face-up card
 *  - during discard, randomly discard a valid card regardless of melds
 *  - if our deadwood is low enough, randomly decide whether or not we knock
 ...
 */
public class BasicMonteCarloPlayer implements GinRummyPlayer {
	...
	public boolean willDrawFaceUpCard(Card card) {
		...
		//		return true;
		*/
		boolean will = random.nextBoolean();
		return will;
	...
	public Card getDiscard() {
		...
		for (Card card : cards) {
			...
			*/
			//All valid cards are candidates.
			candidateCards.add(card);
		}
	...
	public ArrayList<ArrayList<Card>> getFinalMelds() {
		...
		//If the opponent hasn't knocked, but we have no melds nor sufficient melds that our deadwood is low enough, then we won't knock
		...
		//If the opponent has knocked, proceed to return your final meld
		ArrayList<ArrayList<Card>> final_meld; 
		final_meld = bestMeldSets.isEmpty() ? new ArrayList<ArrayList<Card>>() : bestMeldSets.get(random.nextInt(bestMeldSets.size()));
		
		//Now decide whether or not we knock, if we're allowed to, or merely
		//submit final melds if our opponent has already knocked
		return (opponentKnocked || random.nextBoolean()) ? final_meld : null;
